...
class ALAC(object):
    def __init__(self,
        ...
        with tf.variable_scope('Actor'):
            ...
            alpha3 = (1-self.labda)
            l_derta_1 = tf.reduce_mean(self.l_ - self.l + (alpha3) * ( self.l - 0 ))
            l_derta_2 = tf.reduce_mean(self.l_ - self.l + (alpha3) * ( self.l- self.l_))
            self.l_derta = (1-self.labda) * l_derta_1 + self.labda * l_derta_2


            labda_loss = -tf.reduce_mean(log_labda * tf.stop_gradient(self.l_derta))
    ...
    def _build_l(self, s, a, reuse=None, custom_getter=None):
        ...
        with tf.variable_scope('Lyapunov', reuse=reuse, custom_getter=custom_getter):
            ...
            norm_s = tf.reshape(tf.div(s,(tf.reduce_sum(s,axis=1)+1e-8)[:,tf.newaxis]),[-1,self.s_dim,1])
            norm_s = tf.reduce_sum(norm_s,axis=1)
            one_s = tf.ones_like(norm_s)
            zero_s = tf.zeros_like(norm_s)
            norm_s = tf.where(norm_s < 0.000001, x=zero_s, y=one_s)
            layers[-1] = layers[-1] * norm_s
...
def train(variant):
    ...
    policy = ALAC(a_dim,s_dim, policy_params)
    ...
    for i in range(max_episodes):
        ...
        for j in range(max_ep_steps):
            ...
            # a = a*0
            "caculate the action using normalization, restrict the raw action within [-1,1]"
